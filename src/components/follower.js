        ...
        <div>
            <Header/>
            {
                isLoading ? <p className="mt-5 pt-5 p-2">fetching...</p>:(
                    <div className="mt-5 pt-5 p-2">
                    {
                        followers && (
                            followers.map((follow,index)=>{
                                return(
                                    <div  key={index} className="d-flex flex-row align-items-center mb-1" onClick={(e)=>{navigateToFollower({follow})}}>
                    ...
                    {followers.length ===0?<p className="text-center pt-5 mt-5">You currently have no follower</p>:null}
